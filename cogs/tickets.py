...
class Tickets(commands.Cog):
    ...
    async def new(self, ctx, *, reason = None):
        print(await self.client.db.fetch("SELECT * FROM tickets"))
        ...
        try:
            ...
            if msg.content != "confirm":
                ...
                warning = f"""{ctx.author.mention} it is good to provide a reason for your inquires with the mods \nNext time try `f!new <reason>`
                """
                tickets = await self.client.db.fetchval("SELECT guild_id FROM tickets WHERE guild_id=$1",ctx.guild.id)
                guild = ctx.guild
                author = ctx.author
                # Logic
                if not tickets:
                ...
                ticketrole = await self.client.db.fetchval("SELECT role_id FROM tickets WHERE guild_id=$1",ctx.guild.id)
                ...
                # send the mod log
                try:
                    channelId = await self.client.db.fetchval("SELECT channel_id FROM tickets WHERE guild_id=$1",ctx.guild.id)
    ...
    async def addticketrole(self, ctx, role : discord.Role = None, *,reason = None):
        ...
        author = ctx.author
        if not tickets:
            await self.client.db.execute("INSERT INTO tickets (guild_id, channel_id, role_id) VALUES ($1, $2, $3)", guild.id, None, role.id)
        else:
            await self.client.db.execute("UPDATE tickets SET role_id=$2 WHERE guild_id=$1", guild.id, role.id)
    ...
    async def setticketlogs(self, ctx, channel : discord.TextChannel = None, *, reason = None):
        ...
        if not tickets:
            await self.client.db.execute("INSERT INTO tickets (guild_id, channel_id, role_id) VALUES ($1, $2, $3)", guild.id, channel.id, None)
        else:
            await self.client.db.execute("UPDATE tickets SET channel_id=$2 WHERE guild_id=$1", guild.id, channel.id)
    ...
    async def close(self, ctx, *, reason = None):
        ...
        if name.startswith("ticket-"):
            ...
            messageEmbed.set_footer(text = "Invite me to your server! f!invite)")
    ...
    async def open_guild(self, guild, ticketrole : discord.Role, ticketchannel: discord.TextChannel):
        ...
        check = await self.client.db.fetchrow("SELECT guild_id FROM tickets WHERE guild_id=$1",guild.id)
        if check:
            await self.client.db.execute("UPDATE tickets SET channel_id=$2,role_id=$3 WHERE guild_id=$1", guild.id, ticketchannel.id, ticketrole.id)
        else:
            self.client.db.execute("INSERT INTO tickets (guild_id, channel_id, role_id) VALUES ($1, $2, $3)", guild.id, ticketchannel.id, ticketrole.id)
def setup(client):
